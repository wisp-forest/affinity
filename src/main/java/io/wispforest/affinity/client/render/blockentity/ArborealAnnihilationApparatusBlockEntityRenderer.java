...
public class ArborealAnnihilationApparatusBlockEntityRenderer extends AffinityBlockEntityRenderer<ArborealAnnihilationApparatusBlockEntity> {
    ...
    protected void render(ArborealAnnihilationApparatusBlockEntity entity, float tickDelta, float frameDelta, long time, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        modelData.getRoot().addChild("main",
                ModelPartBuilder.create().uv(0, 0).cuboid(-1f, -1f, -1f, 2f, 2f, 2f),
                ModelTransform.pivot(0f, 0f, 0f));

        var part = modelData.getRoot().createPart(16, 16);
        var color = Color.ofRgb(AffinityWispTypes.VICIOUS.color());
        ...
        part.render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityTranslucent(WispEntityRenderer.TEXTURE)), light, overlay, color.red(), color.green(), color.blue(), 1f);
        matrices.pop();
