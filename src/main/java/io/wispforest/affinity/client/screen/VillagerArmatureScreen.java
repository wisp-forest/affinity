...
public class VillagerArmatureScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    private final Interpolator scale = new Interpolator(3.5);
    ...
    public void tick() {
        super.tick();
        this.mainMesh.scheduleRebuild();
        this.peripheralMesh.scheduleRebuild();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.mainMesh.canRender() && this.peripheralMesh.canRender()) {
            ...
            RenderSystem.runAsFancy(() -> {
                ...
                var facing = this.armature.facing();
                ...
                var rayOrigin = this.armature.raycastOrigin();
    ...
    private void renderBlockOutline(BlockView world, BlockHitResult blockHit, MatrixStack matrices) {
        var outlineConsumer = this.client.getBufferBuilders().getEntityVertexConsumers().getBuffer(CuboidRenderer.OUTLINE_LAYER);
    ...
    private void renderMeshAndBlockEntities(MatrixStack matrices, WorldMesh mesh) {
        ...
        mesh.renderInfo().blockEntities().forEach((blockPos, entity) -> {
            ...
            this.client.getBlockEntityRenderDispatcher().render(entity, this.client.getRenderTickCounter().getTickDelta(false), matrices, this.client.getBufferBuilders().getEntityVertexConsumers());
            matrices.pop();
        });
    ...
    private void renderTargetMarker(MatrixStack matrices, Vec3d pos, Identifier modelId) {
        ...
        this.renderBakedModel(this.client.getBakedModelManager().getModel(modelId), matrices, consumer);
        matrices.pop();
    }
