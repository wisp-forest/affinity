...
public class VillagerArmatureScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    private void sendProperties() {
        AffinityNetwork.CHANNEL.clientHandle().send(new VillagerArmatureBlockEntity.MyDeathIsImminent(new VillagerArmatureBlockEntity.SetPropertiesPacket(this.armature)));
    }
    ...
    private class MeshesComponent extends BaseComponent {
        ...
        public MeshesComponent(VillagerArmatureBlockEntity armature) {
            ...
            this.mainMesh = new WorldMesh.Builder(this.mainView, mainPositions.get(0), mainPositions.get(mainPositions.size() - 1)).build();
            ...
            this.xSize = this.peripheralMesh.dimensions().getXLength() + 1;
            this.ySize = this.peripheralMesh.dimensions().getYLength() + 1;
            this.zSize = this.peripheralMesh.dimensions().getZLength() + 1;
        }
        ...
        public void draw(OwoUIDrawContext context, int mouseX, int mouseY, float partialTicks, float delta) {
            ...
            if (this.mainMesh.canRender() && this.peripheralMesh.canRender()) {
                ...
                final var lightTransform = new Matrix4f(modelViewStack.peek().getPositionMatrix());
        ...
        private void renderMeshAndBlockEntities(MatrixStack matrices, WorldMesh mesh) {
            ...
            mesh.renderInfo().blockEntities().forEach((blockPos, entity) -> {
                ...
                client.getBlockEntityRenderDispatcher().render(entity, client.getTickDelta(), matrices, client.getBufferBuilders().getEntityVertexConsumers());
                matrices.pop();
            });
        ...
        private void renderBakedModel(BakedModel model, MatrixStack matrices, VertexConsumer vertices) {
            ...
            for (var direction : Direction.values()) {
                ...
                for (var quad : model.getQuads(null, direction, random)) {
                    vertices.quad(matrices.peek(), quad, 1f, 1f, 1f, LightmapTextureManager.MAX_LIGHT_COORDINATE, OverlayTexture.DEFAULT_UV);
                }
            }
