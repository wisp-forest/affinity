...
public class VillagerArmatureBlockEntity extends AethumNetworkMemberBlockEntity implements InteractableBlockEntity, TickedBlockEntity, InWorldTooltipProvider {
    ...
    public static final KeyedEndec<VillagerData> VILLAGER_DATA = CodecUtils.toEndec(VillagerData.CODEC).keyed("villager_data", new VillagerData(VillagerType.PLAINS, VillagerProfession.NONE, 1));
    private static final KeyedEndec<ItemStack> HELD_STACK = BuiltInEndecs.ITEM_STACK.keyed("held_stack", ItemStack.EMPTY);
...
//    }
    ...
    public ActionResult useItem(FakePlayer player) {
        // TODO reach??
        var entityHit = InteractionUtil.raycastEntities(player, 1f, 5, .25f, Entity::isAlive);
        if (entityHit != null) {
    ...
    private @Nullable BlockHitResult raycastBlock(FakePlayer player) {
        var fakeHit = player.raycast(5, 1f, false);
...
//        entries.add(Entry.text(Text.empty(), Text.literal()));
    ...
    public void readNbt(NbtCompound nbt) {
        super.readNbt(nbt);
        this.setHeldStack(nbt.get(HELD_STACK));
    ...
    protected void writeNbt(NbtCompound nbt) {
        super.writeNbt(nbt);
        nbt.put(HELD_STACK, this.heldStack);
    ...
    public static void initNetwork() {
        ...
        PacketBufSerializer.register(
            SetPropertiesPacket.class,
            (buf, packet) -> buf.write(SetPropertiesPacket.ENDEC, packet),
            buf -> buf.read(SetPropertiesPacket.ENDEC)
        );

        // TODO set properties packet
        AffinityNetwork.CHANNEL.registerServerbound(MyDeathIsImminent.class, (death, access) -> {
            var message = death.dies;
            if (!(access.player().getWorld().getBlockEntity(message.armaturePos) instanceof VillagerArmatureBlockEntity armature)) return;
    ...
    public record MyDeathIsImminent(SetPropertiesPacket dies) {}
    ...
    public record SetPropertiesPacket(BlockPos armaturePos, Action action, RedstoneMode redstoneMode, Vec2f clickPosition, boolean sneak) {
        public static final StructEndec<SetPropertiesPacket> ENDEC = StructEndecBuilder.of(
            BuiltInEndecs.BLOCK_POS.fieldOf("armature_pos", SetPropertiesPacket::armaturePos),
