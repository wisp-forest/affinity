...
import io.wispforest.affinity.mixin.access.RecipeManagerAccessor;
...
public class AffinityEmiPlugin implements EmiPlugin {
    ...
    public void register(EmiRegistry registry) {
        ...
        ((RecipeManagerAccessor) registry.getRecipeManager()).affinity$getAllOfType(AffinityRecipeTypes.ASPEN_INFUSION).forEach((id, recipe) -> {
            registry.addRecipe(new AspenInfusionEmiRecipe(id, recipe));
        ...
        ((RecipeManagerAccessor) registry.getRecipeManager()).affinity$getAllOfType(AffinityRecipeTypes.POTION_MIXING).forEach((id, recipe) -> {
            registry.addRecipe(new PotionMixingEmiRecipe(id, recipe));
        ...
        ((RecipeManagerAccessor) registry.getRecipeManager()).affinity$getAllOfType(AffinityRecipeTypes.SPIRIT_ASSIMILATION).forEach((id, recipe) -> {
            registry.addRecipe(new SpiritAssimilationEmiRecipe(id, recipe));
        ...
        ((RecipeManagerAccessor) registry.getRecipeManager()).affinity$getAllOfType(AffinityRecipeTypes.ASSEMBLY).forEach((id, recipe) -> {
            if (recipe instanceof ShapedAssemblyRecipe shaped) {
                registry.addRecipe(new AssemblyEmiRecipe(
                        AssemblyEmiRecipe.padShapedIngredients(shaped),
                        EmiStack.of(shaped.getOutput(MinecraftClient.getInstance().world.getRegistryManager())),
                        id,
                        false
                ));
            } else if (recipe instanceof ShapelessAssemblyRecipe shapeless) {
                registry.addRecipe(new AssemblyEmiRecipe(
                        shapeless.getIngredients().stream().map(AffinityEmiPlugin::veryCoolFeatureYouGotThereEmi).toList(),
                        EmiStack.of(shapeless.getOutput(MinecraftClient.getInstance().world.getRegistryManager())),
                        id,
                        true
                ));
            }
        ...
        ((RecipeManagerAccessor) registry.getRecipeManager()).affinity$getAllOfType(AffinityRecipeTypes.ORNAMENT_CARVING).forEach((id, recipe) -> {
            registry.addRecipe(new OrnamentCarvingEmiRecipe(id, recipe));
