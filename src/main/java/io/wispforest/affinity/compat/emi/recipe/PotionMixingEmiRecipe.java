...
public class PotionMixingEmiRecipe extends BasicEmiRecipe {
    ...
    public PotionMixingEmiRecipe(Identifier id, PotionMixingRecipe recipe) {
        ...
        this.inputs = Stream.concat(
                this.recipe.getItemInputs().stream().map(ingrediente -> AffinityEmiPlugin.veryCoolFeatureYouGotThereEmi(ingrediente.itemPredicate())),
                this.recipe.getEffectInputs().stream().map(StatusEffectEmiStack::new)
        ).toList();
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        for (var effect : this.recipe.getEffectInputs()) {
            effectContainer.child(adapter.wrap(
                    (x, y) -> new StatusEffectSlotWidget(effect, x, y).drawBack(false)
            ).margins(Insets.of(1)));
        }

        if (!(this.recipe.getEffectInputs().isEmpty() || this.recipe.getItemInputs().isEmpty())) {
        ...
        var inputs = this.recipe.getItemInputs();
        inputs:
        for (int row = 0; row < MathHelper.ceilDiv(inputs.size(), 3); row++) {
            ...
            for (int column = 0; column < 3; column++) {
                ...
                rowContainer.child(adapter.wrap(
                        (x, y) -> AffinityEmiPlugin.slot(EmiIngredient.of(inputs.get(idx).itemPredicate()), x, y)
                ).margins(Insets.of(1)));
            }
        }
