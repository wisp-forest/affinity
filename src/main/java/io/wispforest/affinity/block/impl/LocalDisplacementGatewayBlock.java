...
public class LocalDisplacementGatewayBlock extends BlockWithEntity {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        var targetPos = pos.offset(hit.getSide().getOpposite());
        if (hit.getSide() == Direction.UP) targetPos = targetPos.down();

        if (!world.isAir(targetPos) || !world.isAir(targetPos.up())) return ActionResult.PASS;
